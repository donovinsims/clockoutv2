    ...
    <main>
      <section className="border-b border-border">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 pt-20 md:pt-28 pb-12 md:pb-16">
          <div className="mono text-[11px] uppercase tracking-[0.22em] text-accent mb-7">Field Notes</div>
      ...
      <section>
        <div className="max-w-7xl mx-auto px-5 sm:px-8 py-12 md:py-16">
          <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-border border border-border overflow-hidden">
            {posts.map((p) => (
              <li key={p.slug} className="bg-background hover:bg-surface transition-colors">
                <Link to={`/blog/${p.slug}`} className="block p-7 md:p-9 group">
                  ...
                  <div className="mt-6 mono text-[11px] uppercase tracking-wider text-muted-foreground">{p.date}</div>
                </Link>
              </li>
            ))}
          </ul>
